...
const JavaSpringBootWebinar = () => {
  ...
  const enrollNow = () => {
    navigate("/course/java-spring-boot-master-course");
  };

  const features = [
    {
      icon: <Code className="w-5 h-5" />,
      text: "Advanced SpringBoot Architecture & Best Practices",
    },
    {
    ...
    },
    {
      ...
      text: "RESTful API Development & Security",
    },
    {
      icon: <Database className="w-5 h-5" />,
      text: "Database Integration & JPA Mastery",
    },
    {
      icon: <Target className="w-5 h-5" />,
      text: "Live Coding Session with Real Projects",
    },
  ];
  ...
  const benefits = [
    {
      ...
      description:
        "Boost your career with in-demand SpringBoot skills that top companies are looking for",
    },
    {
      icon: <GraduationCap className="w-8 h-8" />,
      title: "Expert Knowledge",
      description:
        "Learn directly from professionals who've built large-scale systems at Fortune 500 companies",
    },
    {
      icon: <Wrench className="w-8 h-8" />,
      title: "Hands-on Experience",
      description:
        "Get practical knowledge through live coding sessions and real-world project examples",
    },
    {
      icon: <Rocket className="w-8 h-8" />,
      title: "Industry Insights",
      description:
        "Discover the latest trends and best practices used by leading tech companies",
    },
  ];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            ...
            <div className="space-y-6 mb-6 lg:space-y-8 animate-fade-in-up order-2 lg:order-1">
              ...
              <p className="text-lg sm:text-xl text-white/90 max-w-xl">
                Learn from Industry Experts with 13+ years of experience from
                IBM, Infosys & Amazon
              </p>
              ...
              <ul className="space-y-3 ">
                {features.map((feature, index) => (
                  <li
                    key={index}
                    className="flex items-center text-base sm:text-lg font-medium">
                    <CheckCircle className="w-5 h-5 text-green-400 mr-3 flex-shrink-0" />
                    <span className="hidden sm:inline">{feature.text}</span>
                    <span className="sm:hidden">
                      {feature.text.split(" ").slice(0, 4).join(" ")}...
                    </span>
                  </li>
                ))}
              </ul>
            ...
            <div className="order-1 lg:order-2">
              <div
                className={`bg-white/95 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-6 sm:p-8 lg:p-10 shadow-2xl border border-white/30 relative overflow-hidden transition-transform duration-1000 ${
                  isCardFloating ? "-translate-y-2" : "translate-y-0"
                }`}>
                {/* Shimmer Effect */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {experts.map((expert, index) => (
              <div
                ...
                <div
                  className={`w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br ${expert.gradient} rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 text-white text-2xl sm:text-3xl font-black group-hover:scale-110 transition-transform duration-300`}>
                  {expert.initials}
                </div>
